 ...
 * Title: Door
 * 
 * @author Alex Byrd Date Updated: 8/15/2016
 * 
 *         Description: The Door Entity is an unseeable entity that coorelates
 *         to a wall that is supposed to act as a door. When this entity is
 *         activated it will move the door block up, keep it there for a little
...
public class Door extends Entity {
	...
	/**
	 * A constructor of a door entity, which also holds the coordinates of the block
	 * the door will raise.
	 * 
	 * @param x
	 * @param y
	 ...
	 * @param wallZ
	 */
	public Door(double x, double y, double z, int wallX, int wallZ, int type, int itemActID, double maxHeight) {
		...
		/*
		 * If the item activation id is greater than 0 and not the special ID.
		 */
		...
		// default to opening to a height of 4
	...
	/**
	 * Gets the door block at the location of the door entity, and moves it up to a
	 * height of 3, makes it stay there long enough for the player to move through,
	 * then goes back down.
	 */
	@Override
	...
	public void move() {
		...
		/*
		 * When the door is moving, this will tell the program to adjust the Players
		 * wall detection settings so that when walking inside doorway, the player still
	...
	private void moveUp() {
		...
		// If door is moving down
			...
			double distance = Math.sqrt(((Math.abs(this.getX() - Player.x)) * (Math.abs(this.getX() - Player.x)))
					+ ((Math.abs(this.getZ() - Player.z)) * (Math.abs(this.getZ() - Player.z))));

			/*
			 * Checks to see if the player or an enemy is under the door and if either is
			 * true, do not allow the door to come down and trap the player or enemy under
			 * the door, and make the door detect the Player or enemy under it and go back
			 * up until the object is gone.
			 * 
			 * Because the Player can crouch, making his/her height 0, causing the door to
			 * shut when he/she crouches, the 0.1 is added to the if statement so that the
			 * door does not close when the player is crouched.
			 * 
			 * The check for enemies is below in the for loop.
			 */
			if (distance <= 0.7 && thisBlock.y <= Player.height + 0.1) {
				time = 0;
				return;
			} else {
	...
	/**
	 * If door is supposed to move down, not up. Can also crush player and entities
	 */
	private void moveDown() {
		...
		if (doorBlock.y > maxY && time == 0) {
			...
			/*
			 * Checks to see if the player or an enemy is under the door and if either is
			 * true, crush the player or entity under the door. Make player crouch first
			 * though
			 * 
			 * The check for enemies is below in the for loop.
			 */
			if (distance <= 0.7 && thisBlock.y <= Player.y + Player.height + Player.maxHeight + 0.1
